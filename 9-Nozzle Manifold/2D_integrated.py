...
output_file_path = "9_Nozzle_Automatic.gcode"
...
for h in range(HeightIterations):
    for v in range(VerticalIterations):
        ...
        output_file.write("G4S002\n")
        skein(x, y, 0)
        output_file.write("M107\n")
        output_file.write("G1  X" + str(X) + "  Y" + str(Y) + "  Z13.0000000000000" + "  F300.0000000000000\n")
    for v in range(VerticalIterations):
        operations += 1
        x = 1.5*offset*3*h+offset
        y = 1.5*offset*3*v+offset
        X = f"{x:.13f}"
        Y = f"{y:.13f}"
        output_file.write("G1  X" + str(X) + "  Y" + str(Y) + "  Z10.0000000000000" + "  F300.0000000000000\n")
        output_file.write("G1  X" + str(X) + "  Y" + str(Y) + "  Z0.0000000000000" + "  F120.0000000000000\n")
